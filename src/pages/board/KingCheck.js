...
{  
    ...
    let king_color = Pieces[current_king]["color"]
    piece.map((key) =>
    {   
        let piece_color = key[1]["color"]
        if(king_color !== piece_color)
        {
            let int_xd = parseInt(Pieces[current_king]["position_x"]) // Destination x
            let int_yd = parseInt(Pieces[current_king]["position_y"]) // Destination y
            let int_xs = parseInt(key[1]["position_x"]) // Source x
            let int_ys = parseInt(key[1]["position_y"])
            if(key[0].includes("pawn"))
            {
                if(PawnMove(int_xd,int_xs,int_yd,int_ys,key[0],current_king))
                {
                    con[0] = current_king
                    con[1].push(key[0])
                    con[2] = true
